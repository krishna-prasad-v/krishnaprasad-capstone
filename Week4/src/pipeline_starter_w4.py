...
from helpers import calculate_cost, get_csv_file_path, read_questions_from_csv
load_dotenv()
...
async def ask_llm(question: Question) -> Answer:
    ...
    except AnthropicError as e:   # base class for all SDK exceptions
        logger.exception(f"Anthropic token count error")
        raise

    if count.input_tokens > 50:   # input guardrails
...
# So it is better to handle multiple text blocks.
    ...
    cost = calculate_cost(token_count_in=response.usage.input_tokens,
                                token_count_out=response.usage.output_tokens,
                                model=model)

    return Answer(question=question.text, text="".join(text_parts), cost_usd=cost)
...
async def ask_llm_with_retry(question: Question, max_retries: int = 3) -> Answer:
    for attempt in range(max_retries+1):
        ...
        try:
            ...
            raise  # Exponential backoff not done
        except Exception:
...
# Process question_list in chunks of 5
    for i in range(0, len(question_list), 5):
        ...
        logger.info(f"Pipeline: start of chunk: questions {i + 1} to {i + len(chunk_list)}")
        ...
        for answer in chunk_ans_list:
            if isinstance(answer, Exception):
                ...
                logger.error(f"Failed to process question '{question}': {answer!r}", exc_info=answer)
            else:    
                print(f"Q: {answer.question}\nA: {answer.text}\n Cost: {answer.cost_usd} USD\n")
                logger.info(f"Successfully processed question: {answer.question}")

        logger.info(f"Pipeline: end of chunk: questions: {i + 1} to {i + len(chunk_list)}\n")
